		...
		public void Process(string path, string[] skipFileExtensionsArr)
		{
			...
			finally
			{
				...
				Logger.Log(
					...
					$"Compressed files per minute: {((double)fileCountProcessedByCompactCommand / (double)ts.TotalMinutes).ToString("0.00")}{Environment.NewLine}" +
		...
		private void ProcessFile(FileInfo fi)
		{
			try
			{
				...
				if (fi.Length > 0)
				{
					...
					Interlocked.Add(ref bytesRead, fileDiskSize1);
