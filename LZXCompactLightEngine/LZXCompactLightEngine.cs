        ...
        private void ProcessFile(FileInfo fi)
        {
            try
            {
                ...
                if (fi.Length > 0)
                {
                    ...
                    if (fileDict.TryGetValue(filePathHash, out uint dictFileSize) && dictFileSize == physicalSize1)
                    {
                    ...
                    uint physicalSize2 = DriveUtils.GetPhysicalFileSize(fi.FullName);
                    fileDict[filePathHash] = physicalSize2;
                    
                    long physicalSize2_Clusters = DriveUtils.GetDiskOccupiedSpace(physicalSize2, fi.FullName);
